...
func printUsage(fs *flag.FlagSet) {
	...
	fmt.Fprintln(out, "  update                       install the latest release and restart the service")
	...
	fmt.Fprintln(out)
	fmt.Fprintln(out, "Staying current:")
	fmt.Fprintln(out, "  xdev update                  # install the latest release, restart, roll back on failure")
	fmt.Fprintln(out, "  xdev update --check          # is there a newer release? (changes nothing)")
	fmt.Fprintln(out, "  xdev update --version v0.2.6 # pin, or roll back, to a specific release")
	fmt.Fprintln(out)
